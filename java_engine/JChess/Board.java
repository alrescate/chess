...
public class Board {
    ...
    public void setFromBoard(Board copy) { // implicitly verified by functionality of larger unit (copy constructor)
    	for (int i = 0; i < 64; i++) {
    	...
    	hash = copy.getHash();
    	hashHistory = new ArrayList<Integer>(copy.getHashHistory());
    } 
    
    private void initialize() { // implicitly verified by other tests
    	squares = new int[64];
    	pedigree = new ArrayList<ArrayList<Integer>>();
    	hashHistory = new ArrayList<Integer>(200); // minimize growing by pre-allocating 200 spaces
    }
    ...
    public boolean isBlackControlled(int i) {
    	// check for knights
    	int[] directNJumps = ChessHelpers.getMoveList(ChessHelpers.wKnight, i);
    	for (int s: directNJumps) { // if a black knight can get to the square, black controls it.
    		if (s != 0xff) {
